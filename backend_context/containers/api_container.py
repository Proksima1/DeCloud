...
from base.infrastructure.s3.session import session_from_settings
...
class ApiContainer(Container):
    ...
    async def build_from_settings(cls) -> "ApiContainer":
        ...
        s3_session = session_from_settings()
        s3_repository = S3Repository(_s3_session=s3_session)
        api_service = ApiService(_s3_supplier=s3_supplier, _api_repository=api_repository, _s3_repository=s3_repository)
